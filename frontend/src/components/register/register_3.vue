<template>
    <div>
        <div class="register_hello3">직종과 관련된 키워드 중<br />관심 진로를 선택해 주세요.</div>
        <div style="width: calc(100% + 10px)">
            <div class="register_selectedJob">{{ interestJob }}</div>
            <div class="register_jobTag" v-for="tag in jobTagLists" :key="tag" style="display: inline-block">
                <input type="checkbox" :id="tag" name="jobList" :value="tag" v-model="userJobTags" />
...
export default {
    ...
    data() {
        return {
            ...
            jobTagLists: ["웹프로그래머", "응용프로그래머", "시스템프로그래머", "DBA 데이터베이스", "네트워크.서버.보안", "게임", "웹마케팅", "동영상제작.편집", "QA.테스터.검증", "웹기획.PM"],
        };
    },
    methods: {
        deleteTag(tag) {
